#24/12/2017
#Subject: going thrugh all files image by image

# create a matrix for measurements of all images
# divided based on image orientation: edge view or side view
side_view_data <- matrix(ncol = 51,nrow = 0,byrow = TRUE)
edge_view_data <- matrix(ncol = 51,nrow = 0,byrow = TRUE)
...
project <- list.files('C:/Users/roni/Desktop/Image_processing')

# create a lop that goes thruogh the files (sites) in the directory
for (sites in project) {
  ...
  for (specimen in specimens) {
    ...
    source('C:/Users/roni/Dropbox/Roni_PHD/P.nuttalli/image processing.R')
...
# colnames give each column a name from 1 to 50
colnames(dim_side_view) <- c('width','height(um)','foram_name')
colnames(dim_edge_view) <- c('depth','height(um)','foram_name')
write.csv(dim_side_view,'C:/Users/roni/Dropbox/Roni_PHD/P.nuttalli/dim_side_view.csv')
write.csv(dim_edge_view,'C:/Users/roni/Dropbox/Roni_PHD/P.nuttalli/dim_edge_view.csv')
